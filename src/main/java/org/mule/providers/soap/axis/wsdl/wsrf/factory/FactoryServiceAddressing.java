/*
 * $Id$
 * --------------------------------------------------------------------------------------
 * Copyright (c) MuleSource, Inc.  All rights reserved.  http://www.mulesource.com
 *
 * The software in this package is published under the terms of the CPAL v1.0
 * license, a copy of which has been included with this distribution in the
 * LICENSE.txt file.
 */

package org.mule.providers.soap.axis.wsdl.wsrf.factory;

/**
 * The Interface FactoryServiceAddressing.
 */
public interface FactoryServiceAddressing extends FactoryService
{
    
    /**
     * Gets the factory port type port.
     * 
     * @param reference the reference
     * @return the factory port type port
     */
    FactoryPortType getFactoryPortTypePort(org.apache.axis.message.addressing.EndpointReferenceType reference) throws javax.xml.rpc.ServiceException;
}

